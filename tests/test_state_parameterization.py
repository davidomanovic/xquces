...
def test_composite_reference_ansatz_jacobian_matches_finite_difference():
    norb = 4
    nocc = 2
    nelec = (2, 2)
    ...
    ref_params = np.linspace(0.2, 0.6, doci_dimension(norb, nelec) - 1)
    ansatz_params = np.linspace(0.01, 0.01 * ansatz_parameterization.n_params, ansatz_parameterization.n_params)
    params = np.concatenate([ref_params, ansatz_params])

    jac = make_composite_reference_ansatz_jacobian(composite)(params)
    eps = 1e-7
    for k in range(params.size):
        step = np.zeros_like(params)
        step[k] = eps
        fd = (
            composite.state_from_parameters(params + step)
            - composite.state_from_parameters(params - step)
        ) / (2 * eps)
        assert np.allclose(jac[:, k], fd, atol=1e-5, rtol=1e-5)
