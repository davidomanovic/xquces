...
def _left_complex_givens_to_zero(
...
def _apply_left_complex_givens(
    mat: np.ndarray, p: int, q: int, theta: float, phi: float
) -> np.ndarray:
    c = np.cos(theta)
    s = np.sin(theta)
...
class _GaugeReducedUCJMap:
    def __init__(self, norb: int, n_reps: int, interaction_pairs=None):
        ...
        self.v_aa, self.n_indep_aa = self._build_gauge_basis(
            self.pairs_aa, diag_factor=False
        )
        self.v_ab, self.n_indep_ab = self._build_gauge_basis(
            self.pairs_ab, diag_factor=True
        )
        self.n_orb_rot_reduced = self.n_orb_rot_full
        self.n_full_per_layer = self.n_orb_rot_full + self.n_aa + self.n_ab
        self.n_reduced_per_layer = (
            self.n_orb_rot_reduced + self.n_indep_aa + self.n_indep_ab
        )
    ...
    def reduced_to_full(self, x_reduced: np.ndarray) -> np.ndarray:
        ...
        for _ in range(self.n_reps):
            x_full[iff : iff + self.n_orb_rot_full] = x_reduced[
                ir : ir + self.n_orb_rot_reduced
            ]
            ir += self.n_orb_rot_reduced
            iff += self.n_orb_rot_full
            if self.n_indep_aa > 0:
                x_full[iff : iff + self.n_aa] = (
                    self.v_aa @ x_reduced[ir : ir + self.n_indep_aa]
                )
            ir += self.n_indep_aa
            iff += self.n_aa
            if self.n_indep_ab > 0:
                x_full[iff : iff + self.n_ab] = (
                    self.v_ab @ x_reduced[ir : ir + self.n_indep_ab]
                )
            ir += self.n_indep_ab
            iff += self.n_ab
        return x_full
    ...
    def full_to_reduced(self, x_full: np.ndarray) -> np.ndarray:
        ...
        for _ in range(self.n_reps):
            ...
            iff += self.n_orb_rot_full
            if self.n_indep_aa > 0:
                x_reduced[ir : ir + self.n_indep_aa] = (
                    self.v_aa.T @ x_full[iff : iff + self.n_aa]
                )
            ir += self.n_indep_aa
            iff += self.n_aa
            if self.n_indep_ab > 0:
                x_reduced[ir : ir + self.n_indep_ab] = (
                    self.v_ab.T @ x_full[iff : iff + self.n_ab]
                )
            ir += self.n_indep_ab
            iff += self.n_ab
        return x_reduced
...
class UCJOpGaugeFixed:
    ...
    def __post_init__(self, validate: bool, rtol: float, atol: float):
        ...
        if self.diag_coulomb_mats.ndim != 4 or self.diag_coulomb_mats.shape[1] != 2:
            raise ValueError(
                f"diag_coulomb_mats should have shape (n_reps, 2, norb, norb). Got shape {self.diag_coulomb_mats.shape}."
            )
        if self.orbital_rotations.ndim != 3:
            raise ValueError(
                f"orbital_rotations should have shape (n_reps, norb, norb). Got shape {self.orbital_rotations.shape}."
            )
        if (
        ...
        if self.orbital_rotations.shape != (
            self.diag_coulomb_mats.shape[0],
            norb,
            norb,
        ):
            raise ValueError(
                "orbital_rotations shape was inconsistent with diag_coulomb_mats."
            )
        if (
            self.final_orbital_rotation is not None
            and self.final_orbital_rotation.shape != (norb, norb)
        ):
            raise ValueError(
                "final_orbital_rotation shape was inconsistent with diag_coulomb_mats."
            )
        if not all(
        ...
        if self.final_orbital_rotation is not None and self.nocc is None:
            raise ValueError(
                "nocc must be provided when final_orbital_rotation is present."
            )
    ...
    def n_params(
        ...
        *,
        ...
        gf = _GaugeReducedUCJMap(
            norb=norb, n_reps=n_reps, interaction_pairs=(pairs_aa, pairs_ab)
        )
        n = gf.n_reduced
        if with_final_orbital_rotation:
            if nocc is None:
                raise ValueError(
                    "nocc must be provided when with_final_orbital_rotation is True."
                )
        ...
        *,
        ...
        if len(params) != n_expected:
            raise ValueError(
                ...
                f"Expected {n_expected} but got {len(params)}."
            )
        x_ucj = params[:n_ucj]
        ...
        if with_final_orbital_rotation:
            ...
            final_orbital_rotation = ov_final_unitary(
                final_params, norb, cast(int, nocc)
            )
    ...
    def to_parameters(
        ...
        *,
        ...
        if pairs_ab is None:
            pairs_ab = triu_indices
        gf = _GaugeReducedUCJMap(
            norb=norb, n_reps=self.n_reps, interaction_pairs=(pairs_aa, pairs_ab)
        )
        ...
        for rep in range(self.n_reps):
            ...
            if gf.n_aa > 0:
                x_full[iff : iff + gf.n_aa] = self.diag_coulomb_mats[rep, 0][
                    tuple(zip(*gf.pairs_aa))
                ]
                iff += gf.n_aa
            if gf.n_ab > 0:
                x_full[iff : iff + gf.n_ab] = self.diag_coulomb_mats[rep, 1][
                    tuple(zip(*gf.pairs_ab))
                ]
                iff += gf.n_ab
        ...
        if self.nocc is None:
            raise ValueError(
                "nocc must be present when final_orbital_rotation is present."
            )
    ...
    def _apply_unitary_(
        ...
        return self.as_stock_ucj()._apply_unitary_(
            vec, norb=norb, nelec=nelec, copy=copy
        )
...
class GaugeFixedUCJBalancedDFSeedExact:
    ...
    def build_parameters(
        ...
        UCJOpGaugeFixed, GaugeFixedUCJSpinBalancedParameterizationExact, np.ndarray
    ]:
        stock = UCJOpSpinBalanced.from_t_amplitudes(
        ...
        seed_ansatz = UCJOpGaugeFixed(
            ...
            final_orbital_rotation=None
            if stock.final_orbital_rotation is None
            else np.array(stock.final_orbital_rotation, copy=True),
            nocc=nocc if stock.final_orbital_rotation is not None else None,
            _final_ov_params=None
            if stock.final_orbital_rotation is None
            else ov_params_from_unitary(stock.final_orbital_rotation, nocc),
        )
