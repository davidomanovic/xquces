...
class IGCR23SpinRestrictedSpec:
    ...
    def tau_matrix(self) -> np.ndarray:
        return _ordered_matrix_from_sparse_values(
    ...
    def from_igcr2_ansatz(
        ...
        *,
        ...
        if not ansatz.is_spin_restricted:
            raise TypeError(
                "iGCR23 is currently implemented only for spin-restricted seeds"
            )
    ...
    def from_ucj_ansatz(
        ...
        *,
        ...
        if ansatz.n_layers != 2:
            raise ValueError(
                "iGCR23 UCJ seeding currently expects either 1 or 2 layers"
            )
...
class IGCR23SpinRestrictedParameterization:
    ...
    def ansatz_from_parameters(self, params: np.ndarray) -> IGCR23Ansatz:
        pieces, pair_values, tau_values, omega_values = (
    ...
    def parameters_from_igcr2_ansatz(self, ansatz: IGCR2Ansatz, **kwargs) -> np.ndarray:
        return self.parameters_from_ansatz(
            IGCR23Ansatz.from_igcr2_ansatz(ansatz, **kwargs)
        )
    ...
    def parameters_from_ucj_ansatz(self, ansatz: UCJAnsatz, **kwargs) -> np.ndarray:
        return self.parameters_from_ansatz(
            IGCR23Ansatz.from_ucj_ansatz(ansatz, self.nocc, **kwargs)
        )

    def parameters_from_gcr_ansatz(self, ansatz: GCRAnsatz, **kwargs) -> np.ndarray:
        return self.parameters_from_ansatz(
            IGCR23Ansatz.from_gcr_ansatz(ansatz, self.nocc, **kwargs)
        )
    ...
    def params_to_vec_jacobian(
        ...
        def jac(params: np.ndarray) -> np.ndarray:
            ...
            u3, x3s = _chart_unitary_and_tangents(
                self.right_orbital_chart, pieces["u3"], self.norb, self.nocc
            )
            diagonal = IGCR23SpinRestrictedSpec(
                pair_values=pair_values,
                tau_values=tau_values,
            ...
            psi3 = apply_orbital_rotation(
                reference_vec, u3, norb=self.norb, nelec=nelec, copy=True
            )
