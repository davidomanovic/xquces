...
def _pair_values_from_matrix(pair: np.ndarray) -> np.ndarray:
    ...
    norb = pair.shape[0]
    return np.asarray(
        [pair[p, q] for p, q in _default_pair_indices(norb)], dtype=np.float64
    )
...
def _tau_values_from_matrix(tau: np.ndarray) -> np.ndarray:
    ...
    norb = tau.shape[0]
    return np.asarray(
        [tau[p, q] for p, q in _default_tau_indices(norb)], dtype=np.float64
    )


def _ordered_matrix_from_sparse_values(
    values: np.ndarray, norb: int, pairs: list[tuple[int, int]]
) -> np.ndarray:
...
def _full_sparse_dict(values: np.ndarray, indices) -> dict:
    return {
        idx: float(val)
        for idx, val in zip(indices, np.asarray(values, dtype=np.float64))
    }


def _vector_from_sparse_dict(default_indices, sparse_dict: dict) -> np.ndarray:
    return np.asarray(
        [sparse_dict.get(idx, 0.0) for idx in default_indices], dtype=np.float64
    )
...
def _pair_feature_matrix(norb: int, nelec: tuple[int, int]) -> np.ndarray:
    ...
    return (
        np.column_stack(cols) if cols else np.zeros((n.shape[0], 0), dtype=np.float64)
    )
...
def _cubic_feature_matrix(norb: int, nelec: tuple[int, int]) -> np.ndarray:
    ...
    cols.extend(
        n[:, p] * n[:, q] * n[:, r] for p, q, r in _default_triple_indices(norb)
    )
    return (
        np.column_stack(cols) if cols else np.zeros((n.shape[0], 0), dtype=np.float64)
    )
...
def _apply_spinless_one_body_generator(
    state: np.ndarray, x: np.ndarray, norb: int, nocc: int
) -> np.ndarray:
...
def _apply_one_body_generator_to_state(
    vec: np.ndarray, x: np.ndarray, norb: int, nelec: tuple[int, int]
) -> np.ndarray:
...
@cache
def _ov_chart_basis_matrices(
    norb: int, nocc: int, real_only: bool
) -> tuple[np.ndarray, ...]:
    nvirt = norb - nocc
    mats = []
...
def _chart_unitary_and_tangents(
    chart, params: np.ndarray, norb: int, nocc: int | None = None
    ...
    if isinstance(chart, IGCR2LeftUnitaryChart):
        ...
        raise NotImplementedError(
            "analytic Jacobian is implemented only for IGCR2 left and OV charts"
        )
    u_dag = u.conj().T
    tangents = []
    for b in basis:
...
class IGCR234SpinRestrictedSpec:
    ...
    @property
    def norb(self) -> int:
        return _infer_norb_from_unique_pair_count(
            len(np.asarray(self.pair_values, dtype=np.float64))
        )
    ...
    def pair_matrix(self) -> np.ndarray:
        return _symmetric_matrix_from_values(
            np.asarray(self.pair_values, dtype=np.float64), self.norb, self.pair_indices
        )
    ...
    def tau_matrix(self) -> np.ndarray:
        return _ordered_matrix_from_values(
            self.tau_vector(), self.norb, self.tau_indices
        )
...
class IGCR234Ansatz:
    ...
    def apply(
        ...
        arr = apply_orbital_rotation(
            arr, self.u4, norb=self.norb, nelec=nelec, copy=False
        )
        arr = apply_igcr4_spin_restricted_diagonal(
            arr, self.diagonal.q_diagonal(), self.norb, nelec, copy=False
        )
        arr = apply_orbital_rotation(
            arr, self.u3, norb=self.norb, nelec=nelec, copy=False
        )
        arr = apply_igcr3_spin_restricted_diagonal(
            arr, self.diagonal.t_diagonal(), self.norb, nelec, copy=False
        )
    ...
    def from_igcr2_ansatz(
        ...
        *,
        ...
        if not ansatz.is_spin_restricted:
            raise TypeError(
                "iGCR234 is currently implemented only for spin-restricted seeds"
            )
        pair = ansatz.diagonal.to_standard().pair_params
        tau, omega = spin_restricted_triples_seed_from_pair_params(
            pair, ansatz.nocc, tau_scale=tau_scale, omega_scale=omega_scale
        )
    ...
    def from_ucj_ansatz(
        ...
        *,
        ...
        if ansatz.n_layers != 3:
            raise ValueError(
                "iGCR234 UCJ seeding currently expects either 1 or 3 layers"
            )
        ...
        tau, omega = spin_restricted_triples_seed_from_pair_params(
            pair_t, nocc, tau_scale=tau_scale, omega_scale=omega_scale
        )
        eta, rho, sigma = spin_restricted_quartic_seed_from_pair_params(
            pair_q,
            nocc,
        ...
        return cls(
            ...
            u2=np.asarray(
                layer_j.orbital_rotation.conj().T @ layer_t.orbital_rotation,
                dtype=np.complex128,
            ),
            u3=np.asarray(
                layer_t.orbital_rotation.conj().T @ layer_q.orbital_rotation,
                dtype=np.complex128,
            ),
    ...
    def from_gcr_ansatz(cls, ansatz: GCRAnsatz, nocc: int, **kwargs) -> "IGCR234Ansatz":
        return cls.from_igcr2_ansatz(
            IGCR2Ansatz.from_gcr_ansatz(ansatz, nocc=nocc), **kwargs
        )
...
class IGCR234SpinRestrictedParameterization:
    ...
    def n_tau_params(self):
        return (
            self.cubic_reduction.n_params
            if self.uses_reduced_cubic_chart
            else len(self.tau_indices)
        )
    ...
    def n_eta_params(self):
        return (
            self.quartic_reduction.n_params
            if self.uses_reduced_quartic_chart
            else len(self.eta_indices)
        )
    ...
    def sector_sizes(self) -> dict[str, int]:
        return {
            ...
            "u3": self.n_u3_params,
            "quartic": self.n_eta_params
            if self.uses_reduced_quartic_chart
            else self.n_eta_params + self.n_rho_params + self.n_sigma_params,
    ...
    def _full_diagonal_vectors_from_public(self, params: np.ndarray):
        ...
        pair_sparse = _symmetric_matrix_from_values(
            np.asarray(pieces["j"], dtype=np.float64), self.norb, self.pair_indices
        )
        ...
        if self.uses_reduced_cubic_chart:
            ...
            tau_values = _vector_from_sparse_dict(
                _default_tau_indices(self.norb),
                _full_sparse_dict(pieces["tau"], self.tau_indices),
            )
            omega_values = _vector_from_sparse_dict(
                _default_triple_indices(self.norb),
                _full_sparse_dict(pieces["omega"], self.omega_indices),
            )
        if self.uses_reduced_quartic_chart:
            ...
            eta_values = _vector_from_sparse_dict(
                ...
                _full_sparse_dict(pieces["eta"], self.eta_indices),
            )
            rho_values = _vector_from_sparse_dict(
                _default_rho_indices(self.norb),
                _full_sparse_dict(pieces["rho"], self.rho_indices),
            )
            sigma_values = _vector_from_sparse_dict(
                _default_sigma_indices(self.norb),
                _full_sparse_dict(pieces["sigma"], self.sigma_indices),
            )
        return (
            pieces,
            pair_values,
    ...
    def ansatz_from_parameters(self, params: np.ndarray) -> IGCR234Ansatz:
        (
            ...
            rho_values,
            sigma_values,
        ) = self._full_diagonal_vectors_from_public(params)
    ...
    def parameters_from_ansatz(self, ansatz: IGCR234Ansatz) -> np.ndarray:
        ...
        idx = 0
        n = self.n_u1_params
        out[idx : idx + n] = self.general_orbital_chart.parameters_from_unitary(
            ansatz.u1
        )
        idx += n
        n = self.n_pair_params
        out[idx : idx + n] = np.asarray(
            [pair[p, q] for p, q in self.pair_indices], dtype=np.float64
        )
        idx += n
        n = self.n_u2_params
        out[idx : idx + n] = self.general_orbital_chart.parameters_from_unitary(
        ...
        if self.uses_reduced_cubic_chart:
            n = self.n_tau_params
            reduced = self.cubic_reduction.reduce_full(
                np.zeros(len(_default_pair_indices(self.norb)), dtype=np.float64),
                np.concatenate([tau, omega]),
            )[1]
            ...
            out[idx : idx + n] = np.asarray(
                [tau_mat[p, q] for p, q in self.tau_indices], dtype=np.float64
            )
            idx += n
            n = self.n_omega_params
            omega_dict = {ix: val for ix, val in zip(d.omega_indices, omega)}
            out[idx : idx + n] = np.asarray(
                [omega_dict.get(ix, 0.0) for ix in self.omega_indices], dtype=np.float64
            )
            idx += n
        n = self.n_u3_params
        out[idx : idx + n] = self.general_orbital_chart.parameters_from_unitary(
        ...
        if self.uses_reduced_quartic_chart:
            n = self.n_eta_params
            reduced = self.quartic_reduction.reduce_full(
                np.concatenate([tau, omega]), np.concatenate([eta, rho, sigma])
            )[1]
            out[idx : idx + n] = reduced
            idx += n
        else:
            ...
            out[idx : idx + n] = np.asarray(
                [eta_dict.get(ix, 0.0) for ix in self.eta_indices], dtype=np.float64
            )
            idx += n
            n = self.n_rho_params
            rho_dict = {ix: val for ix, val in zip(d.rho_indices, rho)}
            out[idx : idx + n] = np.asarray(
                [rho_dict.get(ix, 0.0) for ix in self.rho_indices], dtype=np.float64
            )
            idx += n
            n = self.n_sigma_params
            sigma_dict = {ix: val for ix, val in zip(d.sigma_indices, sigma)}
            out[idx : idx + n] = np.asarray(
                [sigma_dict.get(ix, 0.0) for ix in self.sigma_indices], dtype=np.float64
            )
            idx += n
        n = self.n_u4_params
    ...
    def parameters_from_igcr2_ansatz(self, ansatz: IGCR2Ansatz, **kwargs) -> np.ndarray:
        return self.parameters_from_ansatz(
            IGCR234Ansatz.from_igcr2_ansatz(ansatz, **kwargs)
        )

    def parameters_from_igcr3_ansatz(self, ansatz: IGCR3Ansatz, **kwargs) -> np.ndarray:
        return self.parameters_from_ansatz(
            IGCR234Ansatz.from_igcr3_ansatz(ansatz, **kwargs)
        )
    ...
    def parameters_from_ucj_ansatz(self, ansatz: UCJAnsatz, **kwargs) -> np.ndarray:
        return self.parameters_from_ansatz(
            IGCR234Ansatz.from_ucj_ansatz(ansatz, self.nocc, **kwargs)
        )

    def parameters_from_gcr_ansatz(self, ansatz: GCRAnsatz, **kwargs) -> np.ndarray:
        return self.parameters_from_ansatz(
            IGCR234Ansatz.from_gcr_ansatz(ansatz, self.nocc, **kwargs)
        )
    ...
    def params_to_vec_jacobian(
        ...
        pair_cols = [
            _default_pair_indices(self.norb).index(ix) for ix in self.pair_indices
        ]
        if self.uses_reduced_cubic_chart:
            cubic_features = (
                cubic_features_full @ self.cubic_reduction.physical_cubic_basis
            )
        else:
            ...
            cubic_features = np.concatenate(
                [
                    ...
                    cubic_features_full[
                        :, len(tau_all) + np.asarray(omega_cols, dtype=int)
                    ],
                ],
                axis=1,
            )
        if self.uses_reduced_quartic_chart:
            quartic_features = (
                quartic_features_full @ self.quartic_reduction.physical_quartic_basis
            )
        else:
            ...
            quartic_features = np.concatenate(
                [
                    ...
                    quartic_features_full[
                        :, len(eta_all) + np.asarray(rho_cols, dtype=int)
                    ],
                    quartic_features_full[
                        :,
                        len(eta_all) + len(rho_all) + np.asarray(sigma_cols, dtype=int),
                    ],
                ],
                axis=1,
            )
        ...
        def jac(params: np.ndarray) -> np.ndarray:
            ...
            u1, x1s = _chart_unitary_and_tangents(
                self.general_orbital_chart, pieces["u1"], self.norb, self.nocc
            )
            u2, x2s = _chart_unitary_and_tangents(
                self.general_orbital_chart, pieces["u2"], self.norb, self.nocc
            )
            u3, x3s = _chart_unitary_and_tangents(
                self.general_orbital_chart, pieces["u3"], self.norb, self.nocc
            )
            u4, x4s = _chart_unitary_and_tangents(
                self.right_orbital_chart, pieces["u4"], self.norb, self.nocc
            )
            diagonal = IGCR234SpinRestrictedSpec(
                pair_values=pair_values,
                tau_values=tau_values,
            ...
            psi4 = apply_orbital_rotation(
                reference_vec, u4, norb=self.norb, nelec=nelec, copy=True
            )
            psiq = apply_igcr4_spin_restricted_diagonal(
                psi4, qdiag, self.norb, nelec, copy=True
            )
            psi3 = apply_orbital_rotation(
                psiq, u3, norb=self.norb, nelec=nelec, copy=True
            )
            psit = apply_igcr3_spin_restricted_diagonal(
                psi3, tdiag, self.norb, nelec, copy=True
            )
            psi2 = apply_orbital_rotation(
                psit, u2, norb=self.norb, nelec=nelec, copy=True
            )
            psij = apply_igcr2_spin_restricted(
                psi2, diagonal.pair_matrix(), self.norb, nelec, copy=True
            )
            psif = apply_orbital_rotation(
                psij, u1, norb=self.norb, nelec=nelec, copy=True
            )

            def after_u1(v):
                return apply_orbital_rotation(
                    v, u1, norb=self.norb, nelec=nelec, copy=True
                )

            def after_j(v):
                return after_u1(
                    apply_igcr2_spin_restricted(
                        v, diagonal.pair_matrix(), self.norb, nelec, copy=True
                    )
                )
            ...
            def after_u2(v):
                return after_j(
                    apply_orbital_rotation(
                        v, u2, norb=self.norb, nelec=nelec, copy=True
                    )
                )
            ...
            def after_t(v):
                return after_u2(
                    apply_igcr3_spin_restricted_diagonal(
                        v, tdiag, self.norb, nelec, copy=True
                    )
                )
            ...
            def after_u3(v):
                return after_t(
                    apply_orbital_rotation(
                        v, u3, norb=self.norb, nelec=nelec, copy=True
                    )
                )
            ...
            def after_q(v):
                return after_u3(
                    apply_igcr4_spin_restricted_diagonal(
                        v, qdiag, self.norb, nelec, copy=True
                    )
                )
            ...
            for x in x1s:
                cols.append(
                    _apply_one_body_generator_to_state(psif, x, self.norb, nelec)
                )
            for k in pair_cols:
            ...
            for x in x2s:
                cols.append(
                    after_j(
                        _apply_one_body_generator_to_state(psi2, x, self.norb, nelec)
                    )
                )
            ...
            for x in x3s:
                cols.append(
                    after_t(
                        _apply_one_body_generator_to_state(psi3, x, self.norb, nelec)
                    )
                )
            ...
            for x in x4s:
                cols.append(
                    after_q(
                        _apply_one_body_generator_to_state(psi4, x, self.norb, nelec)
                    )
                )
