...
from xquces.ucj.gaugefixed import GaugeFixedUCJBalancedDFSeedV2
...
OUT_CSV = Path(f"output/{molecule}_{basis}_ucj_gaugefixed.csv")
TRACE_CSV = Path(f"output/{molecule}_{basis}_ucj_gaugefixed_trace.csv")
...
def main():
    ...
    for R in bond_distance_range:
        ...
        ucj_seed, ucj_param, x0_seed = GaugeFixedUCJBalancedDFSeedV2(
            t2=ccsd.t2,
            t1=ccsd.t1,
            n_reps=1,
        ).build_parameters()
