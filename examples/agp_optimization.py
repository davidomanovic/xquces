...
n_fci_roots = 1
s2_tol = 1e-4
output = Path(f"output/{molecule}_{basis}_gcr2_agp_reference.csv")
...
def build_agp_initialized_seed(
    reference_parameterization: xquces.AGPStateParameterization,
    ...
    ansatz_params = np.asarray(
        ansatz_parameterization.parameters_from_ucj_ansatz(ucj_ansatz),
        dtype=np.float64,
    )
    ansatz = ansatz_parameterization.ansatz_from_parameters(ansatz_params)
    dim_doci = xquces.doci_dimension(reference_parameterization.norb, nelec)
    ...
    agp_params = reference_parameterization.parameters_from_amplitudes(coeffs)
    return np.concatenate([agp_params, ansatz_params])


def continuation_seed(parameterization, previous, mol, active_mo, fallback):
    if previous is None:
        return np.asarray(fallback, dtype=np.float64)
    previous_params, previous_parameterization, previous_mol, previous_mo = previous
    ref_prev, ans_prev = previous_parameterization.split_parameters(previous_params)
    overlap = orbital_overlap_between(previous_mol, previous_mo, mol, active_mo)
    ansatz_params = parameterization.ansatz_parameterization.transfer_parameters_from(
        ans_prev,
...
def state_energy(parameterization, x, hamiltonian) -> float:
    psi = parameterization.state_from_parameters(x)
    return float(np.real(np.vdot(psi, hamiltonian @ psi)))


def evaluate_state(parameterization, x, norb, nelec, hamiltonian, psi_fci, e_fci):
    psi = parameterization.state_from_parameters(x)
    ...
    with threadpool_limits(limits=threads):
        for r in rs:
            ...
            if previous_record is not None and use_continuation:
                ...
                seed_label = "UCJ+dressed-AGP"
            print(
                f"norb={norb} nelec={nelec} params={parameterization.n_params} (agp={parameterization.n_reference_params}, ansatz={parameterization.n_ansatz_params})",
                flush=True,
            )
